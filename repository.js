...
const MONGO_URL =
  "mongodb://rick127:tempPassFor_plan5!@ds113915.mlab.com:13915/db_temp";
...
function searchDocuments(collection, prop) {
  return new Promise((resolve, reject) => {
    collection.find({ $text: { $search: prop } }).toArray(function(err, data) {
      assert.equal(err, null);
    ...
    const resolvers = {
      ...
      Mutation: {
        ...
        searchProduct: (_, cred) => {
          ...
          //     });
          //   };
          //// name might be the field
          // products.createIndex({name: "text"}, function(err, result) {
          //     console.log(result)
          // })
          ...
          return searchDocuments(products, input.name)

          // return searchDocuments(products, input)
